...
from ml_models import SleepTransformer, SleepCNN, SleepLSTM
...
# ─────────────────────────────────────────────
@router.post("/analyze")
def analyze(file: UploadFile = File(...), models: str = Form("LSTM"), channels: str = Form("5"), classes: str = Form("3")):
    if not file:
        raise HTTPException(status_code=400, detail=str({"error": "No file uploaded"}))

    f = file
    if not file.filename.lower().endswith(".edf"):
        raise HTTPException(status_code=400, detail="Please upload an EDF file")
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str({"error": str(e)}))
...
def extract_features(data: dict = Body(...)):
    ...
    try:
        ...
        if not stages_int or not class_names:
            raise HTTPException(status_code=400, detail=str({"error": "Missing stages data."}))
...
@router.post("/predict_osa")
def predict_osa(data: dict = Body(...)):
    init_osa_predictor()
    if osa_predictor.osa_model is None:
        raise HTTPException(status_code=500, detail=str({"error": "OSA models not initialized."}))
...
def parse_features_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        if fname.endswith(".csv"):
            ...
            if len(df) == 0:
                raise HTTPException(status_code=400, detail=str({"error": "CSV file is empty."}))
            ...
            raise HTTPException(status_code=400, detail=str({"error": f"Unsupported format: {fname}. Use .csv or .xml"}))
...
@router.post("/predict_osa_custom")
def predict_osa_custom(data: dict = Body(...)):
    ...
    try:
        ...
        if not features:
            raise HTTPException(status_code=400, detail=str({"error": "No features provided."}))
...
def list_channels(file: UploadFile = File(...)):
    if not file:
        raise HTTPException(status_code=400, detail=str({"error": "No file"}))
    f = file
    import tempfile, mne
    tmp_fd, tmp_path = tempfile.mkstemp(suffix=".edf")
    os.close(tmp_fd)
